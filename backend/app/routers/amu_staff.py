...
from app.schemas import PublicNeedsAssessmentSubmission, ReferralEmailRequest
    ...
    try:
        ...
        for doc in cursor:
            ...
            out.append({
                ...
                "section_code": class_doc.get("section_code") or "",
                "college": college,
...
def _serialize_public_invitation(doc: dict) -> dict[str, Any]:
    ...
    if referral_reasons.get("mtg_grade_2_5_and_above"):
        ...
        if mtg_value is not None:
            try:
                grade_value = float(mtg_value)
                referral_reason_message = f"You were referred because your midterm grade was {grade_value}."
            except (ValueError, TypeError):
                referral_reason_message = "You were referred because your midterm grade was between 3.0 and 5.0."
        else:
            referral_reason_message = "You were referred because your midterm grade was between 3.0 and 5.0."
    ...
    return {
        ...
        "referral_reason": referral_reason_message,
        "midterm_grade": doc.get("midterm_grade"),
    ...
    try:
        ...
        if student_email:
            action_labels = {
                ...
                "both_mentoring_and_counselling": "Both Mentoring and Counselling",
                "monitoring_only": "Monitoring Only",
                "other_support": "Other Support",
            }
            action_label = action_labels.get(action, action.replace("_", " ").title())
            
            subject = "Support Routing Decision - Academic Early Warning System"
            message = f"""You have been referred to the Academic Mentoring Unit (AMU) for support.

After reviewing your case, the AMU has determined that you would benefit from:

{action_label}

This decision is based on your academic performance and needs assessment. Please follow up with the AMU office for further guidance and support.

If you have any questions, please contact the AMU office.

Thank you,
Academic Early Warning System Team"""
            
            send_student_support_email(student_email, student_name, subject, message)
