...
def _is_recaptcha_enabled() -> bool:
    """Return True only when a non-placeholder reCAPTCHA secret is configured."""
    secret = (os.getenv("RECAPTCHA_SECRET_KEY") or "").strip()
    if not secret:
        return False
    upper_secret = secret.upper()
    if upper_secret.startswith("YOUR_"):
        return False
    if secret in {"your_reCAPTCHA_v2_secret_key", "YOUR_RECAPTCHA_SECRET_KEY"}:
        return False
    return True
