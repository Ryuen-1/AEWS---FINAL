...
def _can_make_prediction(features: dict[str, float | int]) -> tuple[bool, str]:
    """Check if all required data is available for prediction.
    
    For BukSU system:
    - GPA: 0.0 means missing data, >0 means actual GPA
    - Attendance: 0.0% means never attended (actual zero)
    - Midterm: None means not uploaded yet
    - Needs assessment: 0 means no issues
    """
    
    has_midterm = features.get("midterm_grade") is not None
    has_attendance = features.get("attendance_rate") is not None and features.get("attendance_rate") >= 0
    has_gpa = features.get("previous_gpa") is not None and features.get("previous_gpa") > 0  # 0.0 = missing
    has_needs_assessment = (
        features.get("academic_challenge_score") > 0 or 
        features.get("external_factor_score") > 0
    )
    
    # Require at least 2 of the 4 key indicators before predicting
    data_available_count = sum([has_midterm, has_attendance, has_gpa, has_needs_assessment])
    
    if data_available_count >= 2:
        return True, "Sufficient data available"
    else:
        missing = []
        if not has_midterm:
            missing.append("midterm grade")
        if not has_attendance:
            missing.append("attendance")
        if not has_gpa:
            missing.append("overall GPA")
        if not has_needs_assessment:
            missing.append("needs assessment")
        return False, f"Need more data: {data_available_count}/4 indicators available (require at least 2). Missing: {', '.join(missing)}"
...
def _compute_weight_scores(features: dict[str, float | int]) -> dict[str, Any]:
    ...
    # For BukSU: GPA 0.0 means missing data, 5.0 is failing
    # Only count GPA in weight score if we have actual data (not 0.0)
    if previous_gpa > 0 and previous_gpa <= 5.0:
        if previous_gpa >= 3.0:
        ...
        elif previous_gpa >= 5.0:
            academic_weight_score += 4.0  # 5.0 is failing grade
...
def _classify_risk_drivers(features: dict[str, float | int]) -> dict[str, Any]:
    ...
    # For BukSU: GPA 0.0 means missing data, 5.0 is failing
    # Only flag GPA issues if we have actual data (not 0.0)
    if previous_gpa > 0 and previous_gpa <= 5.0:
        if previous_gpa >= 2.25:
            academic_signals.append(f"Previous GPA is concerning at {previous_gpa:.2f}.")
        elif previous_gpa >= 5.0:
            academic_signals.append(f"Previous GPA is failing at {previous_gpa:.2f}.")
...
def _extract_contributing_signals(
    ...
    previous_gpa = float(features.get("previous_gpa", 0.0))
    # For BukSU: GPA 0.0 means missing data, 5.0 is failing
    # Only add GPA signal if we have actual data (not 0.0)
    if previous_gpa > 0 and previous_gpa <= 5.0:
        if previous_gpa >= 2.0:
            add_signal(
                "previous_gpa",
...
def _predict_with_fallback(features: dict[str, float | int]) -> dict[str, float | int | str]:
    ...
    # For BukSU: GPA 0.0 means missing data, 5.0 is failing
    # Skip GPA contribution if it's 0.0 (missing)
    if previous_gpa > 0 and previous_gpa <= 5.0:
        if previous_gpa >= 3.0:
            risk_score += 3
        elif previous_gpa >= 2.5:
...
def predict_student_risk(enrollment: dict[str, Any]) -> dict[str, Any]:
    ...
    can_predict, reason = _can_make_prediction(feature_dict)
    ...
    # Only proceed with prediction if we have enough data
