...
import { ClipboardList, Search, AlertTriangle, LayoutDashboard, AlertTriangle as AlertIcon, TrendingUp, Clock3, Download, CheckCircle2, RefreshCw, Eye, Zap, X, Send } from 'lucide-react'
    ...
    try {
      ...
      for (const refId of uniqueRefIds) {
        try {
          ...
          if (data.prediction === null && data.risk === "Insufficient Data") {
            // Show a warning for insufficient data instead of marking as predicted
            setItems((prev) =>
              prev.map((item) =>
                item.id === refId
                  ? {
                      ...
                      prediction_error: data.reason || "Insufficient data for prediction",
                    }
                  : item,
              ),
            )
            successCount += 1  // Count as processed even if insufficient data
          } else {
            successCount += 1
            setItems((prev) =>
              prev.map((item) =>
      ...
      <div className="relative space-y-4">
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-md shadow-slate-200/50">
          <div className="flex flex-col gap-1 border-b border-slate-100 bg-gradient-to-r from-slate-50/80 to-white px-6 py-4">
            ...
            <p className="text-sm text-slate-500">Review completed responses, run predictions, and export responses as a downloadable file.</p>
          </div>
                            ...
                            {!r.has_needs_assessment ? (
                              ...
                              <>
                                {prediction.prediction === null && prediction.risk === "Insufficient Data" ? (
                                  <div className="space-y-2">
                                    <div className="inline-flex items-center gap-1.5 rounded-full border border-red-200 bg-red-50 px-2.5 py-1 text-[11px] font-medium text-red-700">
                                      <AlertTriangle className="h-3.5 w-3.5" />
                                      <span>Insufficient data</span>
                                    </div>
                                    <div className="text-[10px] text-red-600 max-w-xs">
                                      {prediction.reason || "Grades should be uploaded, attendance should be uploaded, and needs assessment form should be filled out"}
                                    </div>
                                  </div>
                                ) : (
                                  <button
