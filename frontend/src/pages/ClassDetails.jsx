...
function getStudentKey(student) {
  return String(student?.student_id || student?.student_email || student?.student_name || '').trim().toLowerCase()
}
...
export default function ClassDetails() {
  ...
  const [referralError, setReferralError] = useState('') // eslint-disable-line no-unused-vars
    ...
    <DashboardLayout title="Instructor Dashboard" subtitle={instructorSubtitle}>
                  ...
                  <div className="min-w-0">
                    ...
                    {sectionCode && (
                      <p className="text-blue-100 text-xs mt-0.5">
                        Section: <span className="font-semibold">{sectionCode}</span>
                      </p>
                    )}
            ...
            <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden">
              ...
              {rosterLoading ? (
                ...
                <>
                  ...
                  {(() => {
                    const referredStudents = roster.filter(row => row.flagged_for_mentoring)
                    return referredStudents.length > 0 && (
                      <div className="border-b border-slate-200">
                        <div className="px-4 py-2.5 bg-red-50/60 border-b border-red-100">
                          <h3 className="text-xs font-bold text-red-900 flex items-center gap-1.5">
                            <CircleAlert className="w-3.5 h-3.5" />
                            Referred Students ({referredStudents.length})
                          </h3>
                        </div>
                          ...
                          <table className="w-full table-fixed text-left">
                            <thead className="sticky top-0 z-10 bg-slate-50 border-b border-slate-200">
                              <tr>
                                ...
                                <th className="w-[20%] px-4 py-2.5 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Student Number</th>
                                <th className="w-[14%] px-4 py-2.5 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">MTG</th>
                                <th className="w-[15%] px-4 py-2.5 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Attendance %</th>
                                <th className="w-[15%] px-4 py-2.5 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Status</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                              {referredStudents.map((row, index) => (
                                <tr key={`referred-${row.student_id || row.student_name || index}`} className="hover:bg-red-50/30 transition-colors">
                  ...
                  {/* Non-Referred Students Section */}
                  {(() => {
                    const nonReferredStudents = roster.filter(row => !row.flagged_for_mentoring)
                    return nonReferredStudents.length > 0 && (
                      <div>
                        <div className="px-4 py-2.5 bg-green-50/60 border-b border-green-100">
                          <h3 className="text-xs font-bold text-green-900 flex items-center gap-1.5">
                            <CheckCircle2 className="w-3.5 h-3.5" />
                            Non-Referred Students ({nonReferredStudents.length})
                          </h3>
                        </div>
                        <ScrollTableContainer size="regular">
                          <table className="w-full table-fixed text-left">
                            ...
                            <tbody className="divide-y divide-slate-100">
                              {nonReferredStudents.map((row, index) => (
                                <tr key={`non-referred-${row.student_id || row.student_name || index}`} className="hover:bg-green-50/30 transition-colors">
                                  <td className="px-4 py-2.5">
                                    <div className="flex items-center gap-2 text-sm text-slate-900">
      ...
      {uploadingClasslist && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-white rounded-xl shadow-2xl p-8 flex flex-col items-center gap-4">
            ...
            <p className="text-lg font-semibold text-slate-900">Uploading class list...</p>
                        ...
                        <div className="rounded-xl border border-slate-200/80 bg-slate-50/90 p-5 shadow-sm shadow-slate-200/50">
                          ...
                          <div className="mt-3 space-y-2.5 text-sm text-slate-700">
                            ...
                            <div className="rounded-xl border border-slate-200/70 bg-white px-3 py-2.5">
                              ...
                              <p className="mt-1 text-sm text-slate-900">
                                {activeAIStudent.flagged_for_mentoring
                                  ? 'Automatic academic referral (MTG grade 3.0-5.0)'
                                  : 'No referral yet'}
                              </p>
                            </div>
                            <div className="rounded-xl border border-slate-200/70 bg-white px-3 py-2.5">
      ...
      {uploadingClasslist && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-white rounded-xl shadow-2xl p-8 flex flex-col items-center gap-4">
            <div className="w-12 h-12 border-4 border-blue-600 border-t-transparent rounded-full animate-spin" />
            <p className="text-lg font-semibold text-slate-900">Uploading classlist...</p>
            <p className="text-sm text-slate-600">Please wait while we process your file.</p>
          </div>
        </div>
      )}
