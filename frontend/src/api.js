...
/** List all students. Optional search by name/email. */
export async function getStudents(search = '') {
  const params = new URLSearchParams()
  if (search && String(search).trim()) params.set('search', String(search).trim())
  const qs = params.toString()
  const url = `${API_BASE}/api/students${qs ? `?${qs}` : ''}`
  const res = await fetch(url, {
    headers: getAuthHeaders(),
  })
  const data = await res.json().catch(() => ({}))
...
/** Get students who have been referred (flagged for mentoring). */
export async function getReferredStudents(search = '') {
  const params = new URLSearchParams()
  if (search && String(search).trim()) params.set('search', String(search).trim())
  const qs = params.toString()
  const url = `${API_BASE}/api/students/referred${qs ? `?${qs}` : ''}`
  const res = await fetch(url, {
    headers: getAuthHeaders(),
  })
  const data = await res.json().catch(() => ({}))
  if (!res.ok) {
    throw new Error(formatErrorDetail(data.detail) || res.statusText || 'Failed to load referred students')
  }
  return Array.isArray(data) ? data : []
}
...
export async function deleteStudent(studentId) {
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => ({}))
    throw new Error(formatErrorDetail(data.detail) || res.statusText || 'Delete failed')
  }
  return null
}

/** Get current student profile (for student self-view). */
export async function getStudentProfile() {
  const studentUser = JSON.parse(localStorage.getItem('student_user') || '{}')
  const res = await fetch(`${API_BASE}/api/public/students/profile?student_id=${studentUser.student_id || ''}`, {
    headers: {
  ...
  const data = await res.json().catch(() => ({}))
  if (!res.ok) {
    throw new Error(formatErrorDetail(data.detail) || res.statusText || 'Failed to load profile')
  }
  return data
}
...
export async function updateStudentProfile(payload) {
  const studentUser = JSON.parse(localStorage.getItem('student_user') || '{}')
  const res = await fetch(`${API_BASE}/api/public/students/profile`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
    },
...
export async function trainAdminAnalyticsModel() {
  const res = await fetch(`${API_BASE}/api/admin/analytics/train-model`, {
    method: 'POST',
    headers: getAuthHeaders(),
  })
  const data = await res.json().catch(() => ({}))
  if (!res.ok) {
    throw new Error(formatErrorDetail(data.detail) || res.statusText || 'Failed to train model')
  }
  return data
}
...
/** Fetch full institution general report payload for PDF rendering. */
export async function getAdminGeneralReportData() {
  const res = await fetch(`${API_BASE}/api/admin/reports/general-data`, {
    headers: getAuthHeaders(),
  })
  const data = await res.json().catch(() => ({}))
  if (!res.ok) {
    throw new Error(formatErrorDetail(data.detail) || res.statusText || 'Failed to load general report data')
  }
  return data || {}
}
...
export async function downloadAdminReport(reportId, filename = 'report.csv') {
  const url = `${getAdminReportDownloadUrl(reportId)}?format=csv`
  const res = await fetch(url, { headers: getAuthHeaders() })
  if (!res.ok) {
    const data = await res.json().catch(() => ({}))
...
/** Preview a report in modal with auth headers; returns HTML content. */
export async function previewAdminReport(reportId) {
  const url = `${getAdminReportDownloadUrl(reportId)}?format=pdf`
  const res = await fetch(url, { headers: getAuthHeaders() })
  if (!res.ok) {
    const data = await res.json().catch(() => ({}))
    throw new Error(formatErrorDetail(data.detail) || res.statusText || 'Preview failed')
